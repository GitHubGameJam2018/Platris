...
public class PlayerMovement : MonoBehaviour
{
    ...
    bool timerOn = false, doubleJump = false, canRespawn = true, immunity = false;
    ...
    {
        StartTimer();
        if (Input.GetKeyDown(KeyCode.Space) && isGround == true && !timerOn)
        {
        ...
        else if (Input.GetKeyDown(KeyCode.Space) && doubleJump == false && tapTimer > 0 && tapTimer < maxTapDelay && transform.position.y < maxH.y)
        {
            doubleJump = true;
            timerOn = false;
            tapTimer = 0;
